// ═══ DECADENCE AUDIO ENGINE v3 ═══
// Dark ritual sound design. Clean, no crackle, no artifacts.
// Subsonic dread, spectral resonance, organic darkness.
...
function getCtx() {
  if (!ctx) {
    try {
      ...
      // Clean up on page close — prevent audio persisting after tab/browser close
      window.addEventListener('beforeunload', () => {
        try { stopAmbient(); ctx.close(); } catch(e){}
      });
      // Also handle visibility change (mobile tab switching)
      document.addEventListener('visibilitychange', () => {
        if (document.hidden && ambientNodes) {
          ambientNodes.master.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.5);
        } else if (!document.hidden && ambientNodes) {
...
// Dark cavernous reverb impulse response
...
// ═══ AMBIENT DRONE ═══
// Subsonic pressure, dark filtered drone, distant bell.
// Clean and smooth — no noise layers, no crackle.
export function startAmbient() {
  ...
  // Layer 4: Second drone a fifth above, barely audible — adds depth
  const drone2 = c.createOscillator();
  drone2.type = 'sine';
  drone2.frequency.value = 55; // A1
  const drone2Flt = c.createBiquadFilter();
  drone2Flt.type = 'lowpass';
  drone2Flt.frequency.value = 70;
  ...
  // Slow fade in — 5 seconds
  master.gain.linearRampToValueAtTime(1, c.currentTime + 5);

  ambientNodes = { master, sub1, sub2, drone, lfo, bell, bellMod, drone2, droneFlt, bellFlt, drone2Flt, revSend, subG, droneG, bellG, drone2G, bellModG, lfoG };
}
...
export function stopAmbient() {
  ...
  setTimeout(() => {
    try {
      ...
      n.bell.stop(); n.bellMod.stop(); n.drone2.stop();
    } catch(e){}
  }, 2500);
}

// ═══ CARD FLIP ═══
// Soft low thud — pure sine, no noise.
export function playCardFlip() {
  ...
  tone.frequency.linearRampToValueAtTime(30, c.currentTime + 0.06);
  tone.connect(g);

  // Higher click transient — sine pip
  const click = c.createOscillator();
  click.type = 'sine';
  click.frequency.value = 1200;
  click.frequency.linearRampToValueAtTime(300, c.currentTime + 0.01);
  const cg = c.createGain(); cg.gain.value = 0.04;
  ...
  click.stop(c.currentTime + 0.1);
}

// ═══ PAIR SUCCESS ═══
// Dark spectral bell — solemn, not cheerful.
export function playPairSuccess() {
  ...
  const partials = [174.6, 174.6 * 2.76, 174.6 * 5.4];
  const gains = [0.5, 0.2, 0.05];
...
// ═══ PAIR FAIL ═══
// Guttural sub-bass distortion — short, organic.
export function playPairFail() {
  ...
  o.frequency.linearRampToValueAtTime(28, c.currentTime + 0.12);
...
// ═══ SKIP WARNING ═══
// Rising filtered tone — tense, urgent.
export function playSkipWarn() {
  ...
  o.frequency.linearRampToValueAtTime(500, c.currentTime + 0.2);
  const flt = c.createBiquadFilter();
  flt.type = 'lowpass';
...
// Hollow open fifths with vibrato — eerie, luminous.
export function playAngelic() {
  ...
  freqs.forEach((freq, i) => {
    ...
    g.gain.linearRampToValueAtTime(0.07 - i * 0.012, c.currentTime + d + 0.4);
...
// ═══ DEMON CALL ═══
// Physical dread. No noise layers — pure synthesis.
// 1. Infrasonic pressure (19Hz fear frequency)
// 2. FM metallic scrape descending
// 3. Vocal formant resonance (bandpass-filtered oscillator, NOT noise)
...
export function playDemonCall() {
  ...
  // 2. FM metallic scrape — descending, dark
  const scrape = c.createOscillator();
  scrape.type = 'sine';
  scrape.frequency.value = 100;
  scrape.frequency.linearRampToValueAtTime(50, t + 3);
  const scrapeMod = c.createOscillator();
  scrapeMod.type = 'sine';
  scrapeMod.frequency.value = 103;
  scrapeMod.frequency.linearRampToValueAtTime(51, t + 3);
  const scrapeModG = c.createGain();
  scrapeModG.gain.value = 300;
  ...
  scrapeFlt.frequency.value = 250;
  scrapeFlt.frequency.linearRampToValueAtTime(80, t + 3);
  scrapeFlt.Q.value = 2;
  ...
  if (rev) { const sr = c.createGain(); sr.gain.value = 0.4; scrapeG.connect(sr); sr.connect(rev); }
  ...
  // 3. Vocal formant — sawtooth through formant filters (no noise)
  const vocal = c.createOscillator();
  vocal.type = 'sawtooth';
  vocal.frequency.value = 85;
  vocal.frequency.linearRampToValueAtTime(60, t + 3);
  const formantFreqs = [270, 730, 2300];
  const formantQs = [12, 8, 6];
  const formantGains = [0.03, 0.015, 0.005];
  const vocalMaster = c.createGain(); vocalMaster.gain.value = 0;
  vocalMaster.connect(getMaster());
  if (rev) { const vr = c.createGain(); vr.gain.value = 0.5; vocalMaster.connect(vr); vr.connect(rev); }
  formantFreqs.forEach((f, i) => {
    const flt = c.createBiquadFilter();
    flt.type = 'bandpass';
    flt.frequency.value = f;
    flt.frequency.linearRampToValueAtTime(f * 0.65, t + 3);
    flt.Q.value = formantQs[i];
    const fg = c.createGain(); fg.gain.value = formantGains[i];
    vocal.connect(flt); flt.connect(fg); fg.connect(vocalMaster);
  });
  vocalMaster.gain.linearRampToValueAtTime(1, t + 0.8);
  vocalMaster.gain.linearRampToValueAtTime(0.5, t + 2.5);
  vocalMaster.gain.linearRampToValueAtTime(0, t + 3.5);
  vocal.start(t + 0.15); vocal.stop(t + 4);

  // 4. Descending void tones — two beating triangles
  const v1 = c.createOscillator();
  v1.type = 'triangle';
  v1.frequency.value = 200;
  v1.frequency.exponentialRampToValueAtTime(20, t + 3.5);
  const v2 = c.createOscillator();
  v2.type = 'triangle';
  v2.frequency.value = 203;
  v2.frequency.exponentialRampToValueAtTime(20.3, t + 3.5);
  const vFlt = c.createBiquadFilter();
  vFlt.type = 'lowpass';
  vFlt.frequency.value = 350;
  vFlt.frequency.linearRampToValueAtTime(40, t + 3.5);
  const vG = c.createGain(); vG.gain.value = 0;
  v1.connect(vFlt); v2.connect(vFlt);
  vFlt.connect(vG); vG.connect(getMaster());
  vG.gain.linearRampToValueAtTime(0.1, t + 0.5);
  vG.gain.linearRampToValueAtTime(0.05, t + 2.5);
  vG.gain.linearRampToValueAtTime(0, t + 4);
  v1.start(); v2.start();
  v1.stop(t + 4.5); v2.stop(t + 4.5);

  // 5. Impact thud — sine punch, no noise
  const imp = c.createOscillator();
  imp.type = 'sine';
  imp.frequency.value = 60;
  imp.frequency.exponentialRampToValueAtTime(20, t + 0.3);
  const impG = c.createGain(); impG.gain.value = 0.3;
  imp.connect(impG); impG.connect(getMaster());
  if (rev) { const ir = c.createGain(); ir.gain.value = 0.6; impG.connect(ir); ir.connect(rev); }
  impG.gain.exponentialRampToValueAtTime(0.001, t + 0.35);
  imp.start(t + 0.03); imp.stop(t + 0.5);
}

// ═══ ORACLE ZONE TAP ═══
// Struck resonance — clean FM bell per zone.
export function playZoneTap(zone) {
  ...
  mod.stop(c.currentTime + 1.8);
}

// ═══ BUTTON CLICK ═══
// Tiny sine pip — clean, minimal.
export function playClick() {
  const c = getCtx(); if (!c || muted) return;
  const o = c.createOscillator();
  o.type = 'sine';
  o.frequency.value = 800;
  o.frequency.linearRampToValueAtTime(400, c.currentTime + 0.02);
