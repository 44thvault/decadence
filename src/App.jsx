...
const DemonOracle=({result,onClose,onShare})=>{const[vis,setVis]=useState(false);useEffect(()=>{hapticHeavy();setTimeout(()=>setVis(true),100);},[]);if(!result)return null;const ang=result.type==="angelic";const d=result.demon;const ac=ang?"#ffd700":"#ff0044";const Sec=({label,children})=>(<div style={{borderTop:"1px solid "+ac+"18",paddingTop:12,marginBottom:12}}><div style={{color:ac,fontSize:11,letterSpacing:3,marginBottom:6}}>{label}</div><div style={{color:"#ccc",fontSize:16,lineHeight:1.8}}>{children}</div></div>);
