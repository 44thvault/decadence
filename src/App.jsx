...
const DEMONS = {
  ...
  2: { name: "Doogu", mesh: "02", netSpan: "2::1", type: "Cyclic Chronodemon", syzygy: null, zone: "2→1", domain: "Phase-2 Lemur", planet: null, spine: null, description: "Cyclic Chronodemon of the 2::1 net-span. Impulse-entity of the 2nd Phase of Pandemonium.", omen: "Disturbance along the 2→1 passage", power: "Navigation of the 2-1 interval" },
  ...
  4: { name: "Ixigool", mesh: "04", netSpan: "3::1", type: "Amphidemon", syzygy: null, zone: "3→1", domain: "Phase-3 Lemur", planet: null, spine: null, description: "Amphidemon of the 3::1 net-span. Impulse-entity of the 3rd Phase of Pandemonium.", omen: "Disturbance along the 3→1 passage", power: "Navigation of the 3-1 interval" },
  5: { name: "Ixidod", mesh: "05", netSpan: "3::2", type: "Amphidemon", syzygy: null, zone: "3→2", domain: "Phase-3 Lemur", planet: null, spine: null, description: "Amphidemon of the 3::2 net-span. Impulse-entity of the 3rd Phase of Pandemonium.", omen: "Disturbance along the 3→2 passage", power: "Navigation of the 3-2 interval" },
  ...
  44: { name: "Ummnu", mesh: "44", netSpan: "9::8", type: "Amphidemon", syzygy: null, zone: "9→8", domain: "Phase-9 Lemur", planet: null, spine: null, description: "Amphidemon of the 9::8 net-span. Final Phase Limit of all Pandemonium. Ummnu haunts gt-36 and ciphers Djynxx.", omen: "Terminal boundary dissolution", power: "Pandemonium culmination" },
};
...
// #7: removed onClick={onClose} from backdrop — only DISMISS button closes
const DemonOracle=({result,onClose,onShare})=>{const[vis,setVis]=useState(false);useEffect(()=>{hapticHeavy();setTimeout(()=>setVis(true),100);},[]);if(!result)return null;const ang=result.type==="angelic";const d=result.demon;const ac=ang?"#ffd700":"#ff0044";const Sec=({label,children})=>(<div style={{borderTop:"1px solid "+ac+"18",paddingTop:12,marginBottom:12}}><div style={{color:ac,fontSize:12,letterSpacing:3,marginBottom:6}}>{label}</div><div style={{color:"#ddd",fontSize:17,lineHeight:1.85}}>{children}</div></div>);
  return(<div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.94)",display:"flex",alignItems:"center",justifyContent:"center",opacity:vis?1:0,transition:"opacity 0.8s",backdropFilter:"blur(10px)",padding:12}}><div onClick={e=>e.stopPropagation()} style={{maxWidth:420,width:"100%",maxHeight:"90vh",overflowY:"auto",background:"linear-gradient(180deg,#0a0a0a,#050510)",border:"1px solid "+ac+"30",borderRadius:4,padding:"24px 20px",fontFamily:"'Courier New',monospace",boxShadow:"0 0 50px "+ac+"15",position:"relative",WebkitOverflowScrolling:"touch"}}><div style={{position:"absolute",inset:0,pointerEvents:"none",background:"repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(255,255,255,0.015) 2px,rgba(255,255,255,0.015) 4px)"}}/>
    {ang?(<>
      <div style={{color:"#ffd700",fontSize:13,letterSpacing:5,marginBottom:8}}>◈ ANGELIC INDEX ◈</div>
      <div style={{color:"#ffd700",fontSize:42,fontWeight:"bold",marginBottom:6}}>+{result.score}</div>
      <div style={{color:"#888",fontSize:14,lineHeight:1.7}}>Positive result. Your Aeon continues.</div>
    </>):(<>
      <div style={{color:ac,fontSize:13,letterSpacing:5,marginBottom:8}}>◈ DEMON CALL ◈</div>
      <div style={{color:ac,fontSize:32,fontWeight:"bold",marginBottom:4,textShadow:"0 0 15px "+ac+"50"}}>{d.name}</div>
      <div style={{color:"#aaa",fontSize:15,marginBottom:16}}>Mesh-{d.mesh} · {d.type} · [{d.netSpan}]</div>
    ...
    <div style={{minHeight:"100dvh",width:"100%",background:isSub?"#050005":"#000",color:"#ccc",fontFamily:"'Courier New',monospace",position:"relative",overflow:"hidden",WebkitTapHighlightColor:"transparent",transition:"background 0.5s"}}>
      ...
      <div style={{position:"relative",zIndex:2,maxWidth:400,margin:"0 auto",padding:"6px 8px 10px",minHeight:"100dvh",overflow:gamePhase==="menu"?"auto":"auto"}}>
        ...
        <header style={{textAlign:"center",marginBottom:6,paddingTop:4}}>
          <div style={{fontSize:8,letterSpacing:5,color:accent,opacity:0.5,marginBottom:1}}>◈ PANDEMONIUM MATRIX ◈</div>
          ...
          <div style={{fontSize:9,color:accent+"88",letterSpacing:2,marginTop:2}}>{isSub?"THE ULTIMATE BLASPHEMY · PAIRS → 9":"ATLANTEAN TIME-SORCERY · PAIRS → 10"}</div>
        </header>
          ...
          <div style={{textAlign:"center",paddingTop:12}}>
            ...
            {showBrowser&&(<div style={{maxHeight:340,overflowY:"auto",border:"1px solid "+accent+"20",borderRadius:2,padding:"8px",background:"rgba(0,0,0,0.4)",marginBottom:16,textAlign:"left"}}>
              ...
              {Object.values(DEMONS).map(d=>(
                <div key={d.mesh} onClick={()=>{haptic();setOracleResult({type:"demonic",score:parseInt(d.mesh),demon:d});}} style={{padding:"6px 8px",borderBottom:"1px solid #1a1a1a",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                  ...
                  <span style={{color:"#444",fontSize:10}}>{d.type.split(" ").pop()}</span>
                </div>
              ))}
            </div>)}
            ...
            <div style={{padding:"14px 12px",textAlign:"left",border:"1px solid "+(isSub?"#1a001a":"#1a1a1a"),borderRadius:2,background:isSub?"rgba(20,0,20,0.3)":"rgba(0,0,0,0.3)",marginBottom:16}}>
              <div style={{color:accent,fontSize:11,letterSpacing:3,marginBottom:8}}>RULES</div>
              <div style={{color:"#ccc",fontSize:15,lineHeight:1.9,fontFamily:"'Courier New',monospace"}}>{isSub?"40 cards (1-9 x 4 suits + 4 Queens valued 0). Five dealt face-up on the Atlantean Cross, five face-down. Reveal Set-2 cards and pair with Set-1 to sum to 9 (Numogram Syzygies). Pairs score by difference. Unpaired cards penalize. Negative results call demons.":"36 cards (1-9 x 4 suits). Five face-up on the Atlantean Cross (Set-1), five face-down (Set-2). Reveal and pair to sum to 10. Pairs score by difference (5+5=0, 9+1=8). Unpaired cards penalize. An Aeon ends on first negative result."}</div>
            </div>
            ...
            {showAbout&&(<div style={{padding:"14px 12px",textAlign:"left",border:"1px solid #1a1a1a",borderRadius:2,background:"rgba(0,0,0,0.3)",marginTop:8,marginBottom:16}}>
              ...
              <div style={{color:"#ccc",fontSize:14,lineHeight:1.9}}>
                Decadence is a card game derived from the Pandemonium Matrix, a system of Lemurian demonism and time-sorcery developed by the Cybernetic Culture Research Unit (CCRU) at the University of Warwick in the late 1990s.
              </div>
              <div style={{color:"#ccc",fontSize:14,lineHeight:1.9,marginTop:10}}>
                The CCRU's system is built on the Numogram (or Decimal Labyrinth) — a time-map composed of ten zones (0-9) grouped into five syzygies by nine-sum twinning. The 45 demons of the Pandemonium Matrix occupy the mesh positions defined by all possible zone-pairings. Five great syzygetic demons (Katak, Djynxx, Oddubb, Murrumur, Uttunul) carry the fundamental currents of time.
              </div>
              <div style={{color:"#ccc",fontSize:14,lineHeight:1.9,marginTop:10}}>
                The Atlantean Cross layout and the pairing mechanics come directly from the CCRU's original game specifications. Subdecadence adds Queens (valued 0) and pairs to 9 — corresponding to the Numogram's syzygetic principle.
              </div>
